...
#ifdef HAVE_BLOCK_STATS_FLAGS
static int lv_read_device_block_info(struct block_device *bdev,
  ...
  ret = virDomainBlockStatsFlags(bdev->dom, bdev->path, NULL, &nparams, 0);
  if (ret < 0 || nparams == 0) {
  ...
  ret = virDomainBlockStatsFlags(bdev->dom, bdev->path, params, &nparams, 0);
  if (ret < 0) {
...
#ifdef HAVE_BLOCK_STATS_FLAGS
    ret = lv_read_device_block_info(bdev, &binfo);
#else
    ret = virDomainBlockStats(bdev->dom, bdev->path, &(binfo.bi),
                              sizeof(binfo.bi));
#endif /* HAVE_BLOCK_STATS_FLAGS */
    if (ret < 0)
      continue;
